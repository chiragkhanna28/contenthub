...
{
    public class DuplicateAssetService
    {
        internal static async Task<List<Asset>> GetDuplicateAssetsAsync()
        {
            ...
            var queryResult = await ContentHubConnector.Client().Querying.QueryAsync(query, loadConfiguration);
            ...
            List<long> masterFileIds)
        {
            ...
            var queryResult = await ContentHubConnector.Client().Querying.QueryAsync(query, entityLoadConfig);

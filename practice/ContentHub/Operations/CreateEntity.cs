...
namespace ContentHub.Operations
{
    internal class CreateEntity
    {
        public static async Task<long> CreateAssetEntity()
        {
            ...
            IEntity asset = await ContentHubConnector.Client().EntityFactory
            ...
            // Create the asset
            var assetId = await ContentHubConnector.Client().Entities.SaveAsync(asset).ConfigureAwait(false);
        ...
        private static async Task CreateFetchJob(long? assetId, Uri resource)
        {
            ...
            await ContentHubConnector.Client().Jobs.CreateFetchJobAsync(fetchJobRequest).ConfigureAwait(false);
        }
    }
}
